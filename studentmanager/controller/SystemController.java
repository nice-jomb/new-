...
public class SystemController {
    ...
    public AjaxResult submitlogin(String username, String password, String code, String type,
        ...
        switch (type){
            ...
            case "4":{
                ...
                teacherhead.setPassword(password);
                teacherhead.setUsername(username);
                Teacherhead trh = teacherheadService.findByTeacherhead(teacherhead);
                if(StringUtils.isEmpty(trh)){
                    ajaxResult.setSuccess(false);
                    ajaxResult.setMessage("用户名或密码错误");
                    return ajaxResult;
                }
                ajaxResult.setSuccess(true);
                session.setAttribute(Const.TEACHERHEAD,trh);
    ...
    public AjaxResult getPhoto(@RequestParam(value = "sid",defaultValue = "0") Integer sid,
        ...
        if(thid!=0){
            Teacherhead teacherhead = teacherheadService.findById(thid);
            ajaxResult.setImgurl(teacherhead.getPhoto());
            return ajaxResult;
        }
    ...
    public AjaxResult editPassword(String password,String newpassword,HttpSession session) {
        ...
        if (usertype.equals("4")) {
            ...
            if (!password.equals(teacherhead.getPassword())) {
                ajaxResult.setSuccess(false);
                ajaxResult.setMessage("原密码错误");
                return ajaxResult;
            }
            teacherhead.setPassword(newpassword);
            try {
                int count = teacherheadService.editPswdByTeacherhead(teacherhead);
                if (count > 0) {
                    ajaxResult.setSuccess(true);
                    ajaxResult.setMessage("修改成功,请重新登录");
                } else {
